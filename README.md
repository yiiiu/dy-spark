...
本项目提供了 Windows 与 Linux 环境下的一键启动与一键暂停/停止脚本，便于日常运维与管理：

### Windows 环境
- **一键启动**：双击运行 `start.bat`。脚本会自动检查 Python 环境、读取 `.env` 端口配置并在后台拉起 Web 服务，服务就绪后自动在浏览器中打开控制台。若服务已在运行，将直接为您打开控制台。
- **一键暂停 / 停止**：双击运行 `stop.bat`。脚本会根据进程记录与端口占用精准终止 Web 服务进程并释放端口资源。

### Linux 环境
- **一键启动**：执行 `./start.sh`（优先识别已托管的 systemd 服务，或在后台执行 `nohup` 启动并记录 PID）。
- **一键暂停 / 停止**：执行 `./stop.sh`（支持终止已注册的 systemd 服务或根据 PID/端口占用安全关闭并释放资源）。
